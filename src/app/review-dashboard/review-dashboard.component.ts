...
export class ReviewDashboardComponent implements OnInit {
  ...
  reviews: any[] = [];
  message = '';
  editingReview: any = null; // New state variable for the review being edited
  ...
  loadReviews() {
    this.http.get<any[]>(`http://localhost:8084/api/reviews/${this.packageId}`)
      ...
      .subscribe({
        next: (reviewsWithResponses) => {
          this.reviews = reviewsWithResponses;
          this.alreadyReviewed = this.reviews.some(r => r.userId === this.userId);
          if (this.alreadyReviewed) {
            // Check if the current user has a review and if it's the one being edited
            const currentUserReview = this.reviews.find(r => r.userId === this.userId);
            if (currentUserReview && this.editingReview && currentUserReview.reviewId === this.editingReview.reviewId) {
                // Do nothing, already in edit mode for this review
  ...
  startEditReview(review: any) {
    this.editingReview = { ...review }; // Create a copy to avoid directly modifying the displayed review
    this.message = ''; // Clear any previous messages
  }

  setEditRating(stars: number) {
    if (this.editingReview) {
      this.editingReview.rating = stars;
    }
  }
  ...
  submitEditReview() {
    ...
    if (this.editingReview.comment.length < 10) {
      this.message = "Edited comment must be at least 10 characters.";
      return;
    }

    this.http.put(`http://localhost:8084/api/reviews/${this.editingReview.reviewId}`, this.editingReview)
      .subscribe({
        next: () => {
        ...
        error: () => this.message = "Failed to update review."
      });
  }
    ...
    if (confirm('Are you sure you want to delete this review?')) {
      this.http.delete(`http://localhost:8084/api/reviews/${reviewId}`)
        .subscribe({
          ...
          error: () => this.message = "Failed to delete review."
        });
    }
  }
